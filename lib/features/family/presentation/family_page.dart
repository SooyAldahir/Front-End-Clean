import 'dart:convert';
import 'package:edi301/features/chat/data/chat_repository.dart';
import 'package:edi301/features/chat/presentation/chat_page.dart';
import 'package:edi301/features/chat/presentation/chat_family_page.dart';
...
import 'package:edi301/features/family/presentation/family_controller_legacy.dart';
...
class _FamilyPageState extends State<FamiliyPage> {
  ...
  final FamilyRepository _familiaApi = FamilyRepository();
  ...
  void _startChat(int idUsuario, String nombre) async {
    final idSala = await ChatRepository().initPrivateChat(idUsuario);
  ...
  Future<FamilyModel?> _fetchFamilyData() async {
    try {
      final int? familyId = await _controller.resolveFamilyId();
      if (familyId == null) return null;
      return await _familiaApi.getById(familyId);
    } catch (e) {
  ...
  // =========================
  Widget _buildIntegrantesCards({required FamilyModel family}) {
