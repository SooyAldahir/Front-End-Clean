import 'package:edi301/shared/models/search_result.dart';
import 'package:edi301/shared/widgets/app_widgets.dart';
import 'package:flutter/material.dart';
import 'package:edi301/shared/models/family.dart';
import 'package:edi301/features/family/data/family_repository.dart';
import 'package:edi301/core/socket/socket_service.dart';
import 'package:edi301/features/admin/data/admin_repository.dart';
import 'admin_viewmodels.dart';
import 'package:edi301/features/search/data/search_repository.dart';
...
class _FamilyDetailPageState extends State<FamilyDetailPage> {
  ...
  String? _error;
  final _membersApi = AdminRepository();
  ...
  Future<void> _fetchFamilyDetails(int familyId) async {
    try {
      ...
      final familyData = await repo.getById(familyId);
      if (mounted) {
        setState(() {
          _family = familyData;
          _isLoading = false;
  ...
  Future<void> _handleDeleteMember(FamilyMemberModel member) async {
...
class _Section extends StatelessWidget {
  ...
  final Widget Function(FamilyMemberModel item) buildTrailing;
...
class _AddAlumnsSheetState extends State<_AddAlumnsSheet> {
  final _controller = AddAlumnsViewModel();
  final _alumnSearchCtrl = TextEditingController();

  @override
  void initState() {
    super.initState();
    _controller.selectedFamily.value = widget.family;
  }
    ...
    return Column(
      children: [
        ...
        ValueListenableBuilder<List<UserMiniModel>>(
          valueListenable: _controller.alumnResults,
  ...
  Widget _buildSelectedAlumnsList() {
    return ValueListenableBuilder<List<UserMiniModel>>(
              ...
              children: alumns
                  .map(
                    (alumn) => Chip(
                      ...
                      onDeleted: () =>
                          _controller.removeAlumn(alumn as UserMiniModel),
                    ),
                  )
                  .toList(),
            ),
        ...
        builder: (context, isLoading, child) {
          return ElevatedButton.icon(
            ...
            onPressed: isLoading
                ? null
                : () async {
                    final err = await _controller.saveAssignments();
