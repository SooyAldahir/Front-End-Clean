...
class ReporteFamiliasService {
  ...
  Future<FamilyModel> _fetchReporteIndividualData(int familiaId) async {
    final familia = await _familiaApi.getById(familiaId);
    if (familia == null) {
  ...
  pw.Widget _buildTablePadres(FamilyModel familia) {
    return pw.Table(
      border: pw.TableBorder.all(color: PdfColors.grey600, width: 1),
      columnWidths: {
  ...
  pw.Widget _buildTableMiembros(String title, List<FamilyMemberModel> miembros) {
    final headers = [
      'Matricula',
      'Nombre',
      'Apellido',
